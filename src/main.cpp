...
int main() {
    ...
    auto run_vulkan = [&](const char* shader_path, void* params_ptr, size_t params_size, 
    ...
    // Run MatMul on GPU
    double gpu_matmul_time = run_vulkan("build/vulkan_matmul.spv", &params, sizeof(params), 
    ...
    // Run MSM on GPU (Stub)
    double gpu_msm_time = run_vulkan("build/msm.spv", &params_msm, sizeof(params_msm), 
